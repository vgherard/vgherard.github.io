...
[^1]: The integral representation @eq-gateaux-integral-rep with bounded $\psi _P$ automatically follows if $L$ is (weak-star) continuous and Frechét differentiable [@huber2004robust]. In the most general case, the sense in which @eq-functional-approx is assumed to hold is that of a directional (Gateaux) derivative, and we assume @eq-gateaux-integral-rep to hold for some measurable (not necessarily bounded) function $\psi _P$, which is usually easy to verify in concrete cases.
