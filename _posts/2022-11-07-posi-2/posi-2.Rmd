...
interval for model parameters $\beta_i$. Denote by $\hat M$ the selected model 
and, with abuse of notation, the corresponding set of selected parameters. 
If $ \hat{\text{CI}}_i$ are the confidence intervals for parameters $\beta _i$, we 
require:
$$ 
\text{Pr}(\beta _i \in \hat{\text{CI}}_i\,\,\forall i \in \hat M) \geq 1-\alpha
(\#eq:marginalCoverage)
$$
...
the outcome of selection $\hat M$. If $m$ is the selected model, we require:
$$ 
\text{Pr}(\beta _i \in \hat{\text{CI}}_i\,\,\forall i \in m|\,\hat M=m) \geq 1-\alpha
...
$$
\mathbb E \left( \dfrac{|i \in \hat M \colon \ \beta_i \in \hat{\text{CI}}_i|}{|\hat M|} \right) 
\geq1-q
(\#eq:FCR)
$$
where $|S|$ denotes the cardinality of a set $S$. 

Notice that the random variables in the previous equations are $\hat M$ and 
$\hat{\text{CI}}_i$ (denoted by a hat), whereas the true coefficients $\beta_i$ 
...
$$ 
\text{Pr}(\beta _i \in \hat{\text{CI}}_i\,\,\forall i) \geq 1-\alpha
...
$$ 
(\beta _i \in \hat{\text{CI}}_i\,\,\forall i) = (\beta _i \in \hat{\text{CI}}_i\,\,\forall i \in \hat M) \cap (\beta _i \in \hat{\text{CI}}_i\,\,\forall i \notin \hat M)
$$
which implies that:

$$ 
\text{Pr}(\beta _i \in \hat{\text{CI}}_i\,\,\forall i \in \hat M) \geq \text{Pr}(\beta _i \in \hat{\text{CI}}_i\,\,\forall i) \geq 1-\alpha,
