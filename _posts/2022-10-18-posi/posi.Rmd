...
*"linear model uncertainties after staring into the abyss"*, ... En fin, sparing
you my rock bottom, I eventually stumbled upon the right keywords, and
...
knows; but anyone dropping by is welcome to read, comment and report his
skepticism. 👋
...
re-enters into the game, and what I'm going to ramble about in this and the next 
posts.

[^2]: This is not to say that Selective Inference is always properly accounted
for in scientific practice. A relevant example from the field I come from 
(Particle Physics), is documented in this stimulating reference: 
[@isidori2021significance].

### Plan

My goal for the remainder of this post is to illustrate the mechanisms of selective inference through a simple simulation. In future posts, I would like 
to discuss:

- Some more precise mathematical formulations of the problem, focusing on inference after model selection.
- *How to get away with selection.* Some of the "sophisticated methods" I have
been reading about, coming to the rescue when we can't data-split.
...
$(y_i, x_i)_{i = 1, \,2,\, \dots,\,N}$, and we're asked to study the
...
following modeling procedure:

1.  Fit a full linear model with intercept term,
    $Y = mX + q + \varepsilon$.
2.  Stop if the intercept term has at least 1-$\sigma$ significance
    ($p\text{-value}<32\%$), otherwise:
3.  Fit a model with no intercept, $Y = mX + \varepsilon$.
4.  Report the "naive" 95% confidence intervals for the slope $m$ in the
    last fitted model.

The "naive 95% confidence interval" here is $(\hat m_-, \hat m_+)$, where:
...
Now our question is: how often do the naive CIs \@ref(eq:naiveCISlope)
...
We can check the actual coverage of \@ref(eq:naiveCISlope) through a simulation. Here I'll assume $m = \sigma = 1$, and that the
...
for (b in 1:B) {
	...
	# Is 'q' term is "significant"? If yes, drop 'q' and fit a simpler model
	if (q_pval > 0.32)  { 
		q_dropped[[b]] <- TRUE
		fit <- lm(y ~ x - 1) 
	} else {
...
92%! If this difference from the naive 95% coverage guarantee does not
...
times; This is a relative +60% of failures with respect to a honest 95%
CI.
...
$$
\text {Pr}(m \in \text{CI})  = \text {Pr}(m \in \text{CI}_{q \text{ dropped}}\,\vert\,q \text{ dropped})\cdot \text {Pr}(q \text{ dropped}) +\\ +\text {Pr}(m \in m \in \text{CI}_{q  \text{ kept}}\,\vert\,q \text{ kept})\cdot \text {Pr}(q \text{ kept})
...
$m \in \text{CI}_{q \text{ kept}}$). We can estimate all these
probabilities as:

```{r}
mean(q_dropped)  # Pr(m covered)
...
The mathematical explanation of these observations is provided by Eq. \@ref(eq:coverageDecomposition), while the (hopefully) plain English one in the introductory part of this post. I will conclude with a few parenthetical remarks. 
...
That was all for today. If I haven't managed to bore you to death, please give me a second chance with the next post (ETA: a week or so). There, I will give some mathematical details about how one can formulate the problem of Selective Inference in model-building. For the survivors, my future plan is to review some 
(correct) selective methods I found interesting, including:

- Benjamini-Yekutieli control of False Coverage Rate [@benjamini2005false],
- POSI bounds for marginal coverage [@berk2013valid],
- Data Fission, an elegant generalization of old good data splitting [@https://doi.org/10.48550/arxiv.2112.11079].
- ...whatever I discover in the mean time.
