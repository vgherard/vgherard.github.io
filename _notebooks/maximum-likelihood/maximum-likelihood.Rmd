...
$$
c_P(\theta) = \intop \text dP(x,y)\,\ln \left(\frac{1}{q_{\theta}(y\vert x)}\right),(\#eq:CondCrossEntIntegral)
$$
...
A slightly more contrived but stronger interpretation is obtained if we regard 
the $X$ regressors as fixed and consider the objective function to be:

$$
c_P(\theta\vert \mathbf X = \mathbf x) = \sum _{i=1}^N\intop \text dP(y\vert x_i)\,\ln \left(\frac{1}{q_{\theta}(y\vert x_i)}\right),(\#eq:CondCrossEntIntegral2)
$$
where $\text dP(y\vert x_i)$ denotes the conditional $Y\vert X$ probability 
measure for $X = x_i$. This is the cross-entropy for a batch of $N$ independent 
data points conditional on the regressor values. Again, the arguments leading to 
the CLT \@ref(eq:ThetaCLT) and the information criterion \@ref(eq:TIC) can be 
repeated almost verbatim, where the corresponding estimate targets 
$\theta ^*(\mathbf X = \mathbf x)$ and 
$\mathbb E (c_P(\hat \theta _N\vert \mathbf X =\mathbf x))$, of $\hat \theta _N$ 
and $\text{TIC}$ respectively, are now functions of the regressors. In a setting
with random regressors, the corresponding quantities would be interpreted as
being conditional on the observed $X$ values.
