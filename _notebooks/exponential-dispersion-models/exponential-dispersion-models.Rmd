...
$$
\quad \kappa(\theta) = \ln(\dfrac{1+\sum_{i=1}^K e^{\theta _k}}{K+1}), \quad \theta \in \mathbb R^M,\quad \lambda \in \mathbb N(\#eq:MultEDM),
$$
...
$$
\frac{\text d Q_\lambda^*(z)}{\text d \boldsymbol z} =(K+1)^{-\lambda}\sum_{\boldsymbol i\in \mathbb N ^{M}\,\colon \,\sum _{k=1}^{K}i_k\leq\lambda} \binom{\lambda}{i_1,\,i_2,\dots,i_{K+1}} \delta (\boldsymbol z-\boldsymbol i), (\#eq:MultBaseMeasure)
$$
...
$$
d(y,\hat {\boldsymbol p}) = -2\sum _{k=1}^{K+1}y_k\ln \hat p_k.(\#eq:MultDeviance)
$$
