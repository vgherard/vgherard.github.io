...
$$
\beta (Q) \equiv \arg \min _{\beta '}  \mathbb E_Q[(Y - X\beta ^\prime)^2]=\mathbb E_Q (X^TX)^{-1}\mathbb E_Q(X^T Y), (\#eq:BetaQ)
$$

which depends on the arbitrary measure $Q$. If $Q=P$ is the original joint $XY$ probability measure, we get $\beta(P) \equiv \beta$ as defined by Eq. \@ref(eq:BLP), whereas if $Q = \hat P$ is the empirical $XY$ distribution, we get $\beta (\hat P) \equiv \hat \beta$ of Eq. \@ref(eq:OLS).
...
$$
\mathbf X^T(\mathbf Y - \mathbf X \hat \beta)=0. (\#eq:SampleResidualOrthogonality)
$$
These orthogonality properties, satisfied by the sample and population residuals respectively, are independent of the actual distribution of $X$ and $Y$, and are simple consequences of the definition of $\beta$. In particular, if $X$ contains an intercept term, for example $X^1 = 1$, Eqs. \@ref(eq:PopulationResidualOrthogonality) and \@ref(eq:SampleResidualOrthogonality) imply that population (sample) residuals have vanishing expectation (sample mean).
